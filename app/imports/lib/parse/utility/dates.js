...
export function dates(s='') {
	...
	const cleaned = s
		.replace(/ or .*/,'')
		.replace(/ & .*/,'')
		.replace(/.*asap.*/i, moment.utc().format('DD MM YYYY'))
		.replace(/.*soonest possible.*/i, moment.utc().format('DD MM YYYY'))
	;
	// try a common format eg 01/12/2017 or 01/12/17
	let try1 = moment.utc(cleaned, 'DD MM YYYY');

	// if invalid, try another common format eg 1st December 2017
	if (!try1.isValid()) {
		try1 = moment.utc(cleaned, 'DD MMM YYYY');
	}
